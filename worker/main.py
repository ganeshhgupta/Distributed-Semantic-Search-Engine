...
At runtime the worker does NOT load sentence-transformers.
Embeddings were pre-computed at Docker build time and are baked
into the image under /app/data/shard_{SHARD_ID}/.

Environment variables (set in Dockerfile per worker):
    WORKER_ID     unique name, e.g. "search-worker-1"
    WORKER_PORT   port to listen on (default: 8001)
    SHARD_ID      integer shard this worker owns
    SHARD_COUNT   total number of shards
    DATA_DIR      base dir for pre-baked shard data (default: /app/data)
"""
...
async def startup() -> None:
    ...
    logger.info("Worker %s ready — %d documents indexed", WORKER_ID, _shard_index.document_count)
